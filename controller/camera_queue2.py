...
class MyFrameQueue:
    ...
    def rateOfChange(self, showPlot=False):
        ...
        self.meanDiffList = np.mean(self.diffList)
        self.medianDiffList = np.median(self.diffList)
...
def plot(sec):
    global motion
    time.sleep(5)
    threading.Thread(target=daylight_aprox, args = (2, )).start()
    i=0
    while(FQ.isRunning):
        #print("\n")
...
def getMotionLightCond():
    global light_cond
    global motion
    
    return (light_cond, motion)
